...
def main():
    ...
    envs = make_vec_envs('DartHopperPT-v1', args.seed, args.num_processes,
                         args.gamma, args.log_dir, device, False)# env name changed
...
def learn(env,max_timesteps, 
    ...
    envs = make_vec_envs(env, seed, 8,
                         0.95, log_dir, device, False)
    ...
    rollouts = RolloutStorage(num_step, 8,
                              envs.observation_space.shape, envs.action_space,
                              actor_critic.recurrent_hidden_state_size)
    ...
    for iter in range(100):
        ...
        while True:
            ...
            bad_masks = torch.FloatTensor(
                [[0.0] if 'bad_transition' in info.keys() else [1.0]])
            rollouts.insert(torch.tensor([obs]), recurrent_hidden_states, action,
